...
class CacheInteractionTool:
    ...
    def _initialize_db(self):
        ...
        with self.user_env.begin(write=True) as txn:
            if not txn.stat()['entries']:
                with txn.cursor() as cursor:
                    for user in tqdm(self._iter_file('user_subset.json')):
                        user=user[0]
                        cursor.put(
                            user['user_id'].encode(),
        ...
        with self.item_env.begin(write=True) as txn:
            if not txn.stat()['entries']:
                with txn.cursor() as cursor:
                    for item in tqdm(self._iter_file('item_subset.json')):
                        item=item[0]
                        cursor.put(
                            item['item_id'].encode(),
        ...
        with self.review_env.begin(write=True) as txn:
            if not txn.stat()['entries']:
                for review in tqdm(self._iter_file('review_subset.json')):
                    review=review[0]
                    # Store the review
                    txn.put(
