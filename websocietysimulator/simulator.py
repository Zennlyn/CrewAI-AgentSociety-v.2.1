...
class Simulator:
    ...
    def run_simulation(self, number_of_tasks: int = None, enable_threading: bool = False, max_workers: int = None, time_limitation: float = None, save_dir: str = None) -> List[Any]:
        """
        ...
        Args:
            ...
            save_dir: Directory to save the result of each task. If None, results are not saved individually.
        Returns:
        ...
        if not enable_threading:
            ...
            for index, task in enumerate(task_to_run):
                ...
                if save_dir:
                    ...
                    save_path = os.path.join(save_dir, f"task_{index}_result.json")
            ...
            def process_task(task_index_tuple):
                ...
                try:
                    ...
                    with ThreadPoolExecutor(max_workers=1) as single_task_executor:
                        ...
                        try:
                            output = future.result(timeout=900)  # 15 minutes timeout
                            result = {
                                "task": task.to_dict(),
                                "output": output
                            }
                            ...
                            result = {
                                "task": task.to_dict(),
                                "error": "TimeoutError: Task exceeded 15 minutes threshold."
                            }
                except NotImplementedError:
                    result = {
                        "task": task.to_dict(),
    ...
    def load_simulation_outputs(self, save_dir: str):
        """
        Load simulation outputs from a directory instead of running them.
        """
        import glob
        logger.info(f"Loading simulation outputs from {save_dir}")
        self.simulation_outputs = [None] * len(self.tasks)
        file_paths = glob.glob(os.path.join(save_dir, "task_*_result.json"))
        
        loaded_count = 0
        for file_path in file_paths:
            # Extract index from filename, e.g., task_0_result.json
            basename = os.path.basename(file_path)
            try:
                index_str = basename.split('_')[1]
                index = int(index_str)
                with open(file_path, "r", encoding="utf-8") as f:
                    self.simulation_outputs[index] = json.load(f)
                loaded_count += 1
            except Exception as e:
                logger.error(f"Failed to load {file_path}: {e}")
                
        logger.info(f"Loaded {loaded_count} results from {save_dir}")
        return self.simulation_outputs
    ...
    def _evaluate_simulation(self, ground_truth_data: List[Dict]) -> Dict[str, Any]:
        ...
        simulated_data = []
        for output in self.simulation_outputs:
            if output is not None and 'output' in output:
